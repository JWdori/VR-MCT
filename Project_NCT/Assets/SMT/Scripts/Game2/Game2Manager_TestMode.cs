...
public class Game2Manager_TestMode : MonoBehaviour
{
    public TextMeshPro totalTimeText, stageTimeText, WrongAnswerCntText, stageNumText, CorrectAnswerCntText, DifficultyText, LIFEText, TimePeriodText;
    ...
    public bool Is_check10sec_True = false;
    ...
    public void testModeExplainDestroy()
    {
    ...
    void Update()
    {
        ...
        if(scorepannel == true)
        {
            ...
            DifficultyText.text = "테스트모드";
            if(Is_Stage_Start == true)
            {
                TimePeriodText.text = "이 시간 안에 누르세요   " + timeperiod_start[timeperiod_start_idx] +"초 ~" + (timeperiod_start[timeperiod_start_idx]+2)+"초";
            }
            else{
                TimePeriodText.text = "";
            }
...
//////Update switch case function//////
    IEnumerator IsItRightAnswer(int difficulty)
    {
        ...
        if ((timeperiod_start[timeperiod_start_idx]<=(Time.time - check10sec)) && (timeperiod_start[timeperiod_start_idx]+2>(Time.time - check10sec)))
        {
            TimePeriodCorrect();
            yield return new WaitForSeconds(1f);
...
/////////////////////////////////////////////
        ...
        timeperiod_start_idx = Random.Range(0,timeperiod_start.Length);
